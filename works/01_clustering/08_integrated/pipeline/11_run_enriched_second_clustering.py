...
INPUT_DIR = INTEGRATED_DIR / "intermediate" / "enriched_second_clustering"
OUTPUT_DIR = INTEGRATED_DIR / "intermediate" / "enriched_second_clustering_results"
...
FEATURE_LABELS = {
    ...
    "station_elevation_m": "대여소 표고(m)",
    "elevation_diff_nearest_subway_m": "지하철 대비 고도차(m)",
    "nearest_park_area_sqm": "최근접 공원 면적",
    "distance_naturepark_m": "도시자연공원구역 거리(m)",
    "distance_river_boundary_m": "하천경계 거리(m)",
}


def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame]:
    train = pd.read_csv(INPUT_DIR / "ddri_enriched_cluster_ready_input_train_2023_2024.csv")
    test = pd.read_csv(INPUT_DIR / "ddri_enriched_cluster_ready_input_test_2025.csv")
    return train, test
...
def run_k_search(train: pd.DataFrame) -> pd.DataFrame:
    ...
    out.to_csv(OUTPUT_DATA_DIR / "ddri_enriched_kmeans_search_metrics.csv", index=False)
    return out
...
def fit_model(train: pd.DataFrame, test: pd.DataFrame, k: int):
    ...
    train_out.to_csv(OUTPUT_DATA_DIR / "ddri_enriched_cluster_train_with_labels.csv", index=False)
    test_out.to_csv(OUTPUT_DATA_DIR / "ddri_enriched_cluster_test_with_labels.csv", index=False)
...
def save_summaries(train_labeled: pd.DataFrame) -> None:
    ...
    summary.to_csv(OUTPUT_DATA_DIR / "ddri_enriched_cluster_summary.csv")
    crosstab = pd.crosstab(train_labeled["cluster"], train_labeled["district_hypothesis"])
    crosstab.to_csv(OUTPUT_DATA_DIR / "ddri_enriched_cluster_hypothesis_crosstab.csv")


def plot_k_search(k_search: pd.DataFrame) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    sns.lineplot(data=k_search, x="k", y="inertia", marker="o", ax=axes[0])
    axes[0].set_title("환경 보강 군집화 Elbow")
    sns.lineplot(data=k_search, x="k", y="silhouette", marker="o", ax=axes[1])
    axes[1].set_title("환경 보강 군집화 Silhouette")
    fig.tight_layout()
    fig.savefig(OUTPUT_IMG_DIR / "ddri_enriched_kmeans_elbow_silhouette.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def plot_pca(train_labeled: pd.DataFrame, scaler: StandardScaler) -> None:
    ...
    ax.set_title("환경 보강 군집화 PCA 산점도")
    fig.tight_layout()
    fig.savefig(OUTPUT_IMG_DIR / "ddri_enriched_kmeans_pca_scatter.png", dpi=200, bbox_inches="tight")
    plt.close(fig)


def plot_feature_means(train_labeled: pd.DataFrame, scaler: StandardScaler) -> None:
    raw_summary = train_labeled.groupby("cluster")[FEATURE_COLS].mean().reset_index()
    raw_plot_df = raw_summary.melt(id_vars="cluster", value_vars=FEATURE_COLS, var_name="feature", value_name="value")
    raw_plot_df["feature_label"] = raw_plot_df["feature"].map(FEATURE_LABELS)

    fig, ax = plt.subplots(figsize=(14, 6))
    sns.barplot(data=raw_plot_df, x="feature_label", y="value", hue="cluster", ax=ax)
    ax.set_title("환경 보강 군집별 Feature 평균(원자료)")
    ax.tick_params(axis="x", rotation=35)
    ax.set_xlabel("")
    fig.tight_layout()
    fig.savefig(OUTPUT_IMG_DIR / "ddri_enriched_cluster_feature_means_raw.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    ...
    scaled_plot_df = scaled_summary.melt(id_vars="cluster", value_vars=FEATURE_COLS, var_name="feature", value_name="value")
    scaled_plot_df["feature_label"] = scaled_plot_df["feature"].map(FEATURE_LABELS)

    fig, ax = plt.subplots(figsize=(14, 6))
    sns.barplot(data=scaled_plot_df, x="feature_label", y="value", hue="cluster", ax=ax)
    ax.set_title("환경 보강 군집별 Feature 평균(표준화)")
    ax.tick_params(axis="x", rotation=35)
    ax.set_xlabel("")
    ax.set_ylabel("표준화 평균(z-score)")
    fig.tight_layout()
    fig.savefig(OUTPUT_IMG_DIR / "ddri_enriched_cluster_feature_means.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def save_cluster_map(train_labeled: pd.DataFrame) -> None:
    ...
    for _, row in plot_df.iterrows():
        ...
        popup = folium.Popup(
            (
                ...
                f"지구 가설: {row['district_hypothesis']}<br>"
                f"표고: {row['station_elevation_m']:.1f}m<br>"
                f"도시자연공원구역 거리: {row['distance_naturepark_m']:.1f}m<br>"
                f"하천경계 거리: {row['distance_river_boundary_m']:.1f}m"
            ),
            max_width=320,
        )
        folium.CircleMarker(
    ...
    fmap.save(OUTPUT_DIR / "ddri_enriched_cluster_map.html")
...
def main() -> None:
    ...
    (OUTPUT_DATA_DIR / "ddri_enriched_selected_k.txt").write_text(str(selected_k), encoding="utf-8")
